...
> **这一步在干嘛**：开工前先准备好"这一步最多能算多少 token"（预算）和几个空清单（待会儿把选中的请求装进去）。相当于摆好账本再开始派活。
...
> **这一步在干嘛**：优先伺候"已经在跑的请求"——主要是正在 decode 的（每个只需算 1 个 token），尽量让它们平稳出 token；顺带把正在分块 prefill 的请求往前推一截。显存不够时，就抢占（踢掉）最低优先级的请求来腾地方。
...
>
> 🔗 **回扣[贯穿示例](../PRIMER.md#第三部分一个贯穿全文的玩具示例强烈建议先看懂这个)**：例子里那条请求 `[11,7,4]`，prefill 步在这里算出 `num_new_tokens = 3 - 0 = 3`（整段 prompt）；进入 decode 后每步 `num_new_tokens = 4 - 3 = 1`（只算新 token id=9 那一个）。`token_budget` 相应扣 3、扣 1。
...
> **这一步在干嘛**：RUNNING 伺候完若还有预算，就去"等待队列"拉新请求进来做 prefill（或恢复之前被抢占的请求）。先查 prefix cache 命中、能白嫖多少已算 token，再为剩下的 token 分配 KV block。
...
> **这一步在干嘛**：收尾自检（确认没超 token 预算、没超并发上限），并算出"所有在跑请求共享的公共前缀有多长"——这个数给后面的 cascade attention 用（公共前缀只算一次，见 [模块 14](../14-attention-variants/design.md)）。
...
> **这一步在干嘛**：把上面所有决策装进一个纯数据对象 `SchedulerOutput`（谁算多少 token、用哪些 block、有哪些 spec token、grammar 掩码……）——这就是交给 GPU 那边执行的"工单"。
...
> **这一步在干嘛**：还没真算呢，就先把"已算 token 数"加上去——这样**下一步调度能立刻接着排这个请求的后续 token，不必等 GPU 结果回来**（调度与执行解耦）。万一待会儿发现某些 token 没算成（投机解码被拒），再在阶段 G 退回来。
...
> **这一步在干嘛**：GPU 算完回来了。把新生成的 token 接到各请求后面、检查谁该停（命中 EOS / stop string / 长度上限）；投机解码里被拒的 token 把"已算数"退回去；最后打包成给前端的输出（新 token + 是否结束）。

...
> **这一步在干嘛**：mamba 层不用 KV cache，而是为每条在飞行的序列分配一个固定大小的"状态槽"。这一步在首次 forward 时把跨步的状态张量建好，并在每一步算出"本步这批请求各自用第几个槽"（`state_indices_tensor`）——相当于 KV 那边的 block table，但因为每序列只有一个定长状态，间接寻址退化成一个下标。
...
> **这一步在干嘛**：mamba 层的第一步，把输入 hidden 一次投影成两份——主路 `x`（要走卷积和扫描的）和门控 `gate`（最后用来调制输出）。相当于一个门控 MLP 的入口。
...
> **这一步在干嘛**：先用一个短窗口的因果卷积做"局部混合"（让每个 token 看到前面几个 token）。这里有第一个有状态的地方：`conv_state` 是个滑动窗口缓存。prefill 整段算并把窗口末尾写回状态；decode 只把 1 个新 token 推进滑窗。这是 mamba 两个"状态"之一。
...
> **这一步在干嘛**：算出 SSM 递归要用的三个参数 Δ（时间步）、B、C。关键在于它们**不是固定权重，而是从当前输入算出来的**——这就是 Mamba 的"selective"（选择性）：模型能根据看到的内容，决定多记住或多遗忘些什么。这正是 Mamba 区别于早期固定参数 SSM（S4）的命门。
...
> **这一步在干嘛**：mamba 的核心递归 `h_t = A·h_{t-1} + B·x_t`，把历史信息增量地累加进固定大小的状态 `h`（第二个有状态的地方 `ssm_state`）。prefill 对整段序列扫一遍、把最终状态写回槽；decode 拿出该序列的状态、用 1 个新 token 更新、吐 1 个输出。kernel 靠 `state_indices` 这个下标从状态大数组里选中本序列的状态。
...
> **这一步在干嘛**：朴素 scan 是严格串行的递归（`h_t` 依赖 `h_{t-1}`），prefill 长序列时 GPU 闲置。Mamba-2 的洞察是：把序列切成 chunk，**块内**用矩阵乘并行算（喂饱 GPU）、**块间**只串行传块边界的状态（量很小）。元数据（哪个 token 属哪条序列、chunk 怎么对齐序列边界）在顶层一次性算好、所有 mamba2 层共享。
...
> **这一步在干嘛**：模型构造时，每个 attention 层调 `get_rope` 拿一个 RoPE 实例。工厂按配置（用哪种外推法 linear/yarn/…、是不是 M-RoPE）分派到对应子类，并用全局缓存让同参数的层共享一个实例。各外推法只是重写"怎么算频率"这一个方法，旋转逻辑全继承——这一步还把按位置预算好的 `cos_sin_cache`（查表用）建好。
...
> **这一步在干嘛**：每步 forward 里，在 QKV 投影之后、算 attention 之前，按每个 token 的位置去 `cos_sin_cache` 查出 cos/sin，**原地**把 Q/K 的每个二维子空间旋转对应角度。旋转后两个 token 的点积只依赖它们的相对位置——这就是 RoPE 把相对位置编进 attention 的落点。CUDA kernel 每个 block 处理一个 token。
...
> **这一步在干嘛**：图像/视频 token 有"时间×高×宽"的空间结构，position 不再是一维下标而是三维 `(t,h,w)`。这一步预计算每个多模态 token 的三维 position（文本 token 三维取同值，自动退化成普通 RoPE），decode 时用 `mrope_position_delta` 续接后续文本的位置；forward 里按 `mrope_section` 把 cos/sin 切成三段、每段用不同维度的 position 查出来再拼回去。

...
> **这一步在干嘛**：还在 Python 这层，先拍板这次 decode attention 走 V1（一个 block 扫整条序列）还是 V2（沿 KV 维切成多段并行算再合并）。判据就一句话：序列短、或并行度已经够（`num_seqs*num_heads` 大）就走 V1 省事；否则走 V2 把闲着的 SM 填满。
...
> **这一步在干嘛**：进了 C++ 层，给 kernel 摆好「开多少线程、铺多大 grid、要多少 shared memory」。grid 按 `(head, seq, partition)` 三维铺开（V1 的 partition 维=1，V2=分区数），shared memory 要算好够放 logits 和跨 warp reduce 的暂存（两者复用同一块）。head_size 在编译期用 switch 定死，每个组合编译出独立 kernel。
...
> **这一步在干嘛**：GPU 上真正算 attention。每个 thread block 负责「1 个 head × 1 条序列 × 1 个 KV 分区」：先把 Q 载进 shared memory，然后 warp 分着扫 KV block——**每访问一个 KV block，先查 block table 把逻辑块号翻成物理块号**（PagedAttention 的精髓），几个线程合伙算一个 token 的 Q·K 点积，再两级 reduce 出 softmax，最后加权 V 累加出这个位置的输出。这正是贯穿示例里 decode 步「`q3` 对 pos0~3 的 KV 算注意力」在 GPU 上的样子。
...
```
> 🔗 **回扣[贯穿示例](../PRIMER.md#第三部分一个贯穿全文的玩具示例强烈建议先看懂这个)**：示例的 `block_table=[7,3]` 就是这里取出的「本序列页表」。下面 C6 的 `block_table[block_idx]` 把逻辑块号翻成物理块号——例子里逻辑块 1（含 pos2/pos3）翻成物理块 3，再 `×block_size(2)+偏移` 取到 slot6/slot7 的 KV。kernel 取 KV 全程靠这张页表间接寻址，这就是「逻辑连续、物理可乱放」的 PagedAttention。
...
> **这一步在干嘛**：只有 V2 才有这一步。阶段 C 时 V2 把一条序列切成多段、各 thread block 算出了**各自那段的局部 softmax**（局部 max、局部 exp_sum、局部加权 V）。这个 reduce kernel 把同一 (序列, head) 的所有分段用 online-softmax 公式拼回**全局**结果：先求全局 max，把各段的 exp_sum 按 `exp(局部max - 全局max)` rescale 再相加，最后加权合并各段输出。和 cascade attention、FlashDecoding 是同一套数学。
...
> **这一步在干嘛**：模型每层算出新的 K/V 后，要把它们写进分页 KV cache 的正确位置。每个 token 一个 thread block，按上层算好的 `slot_mapping[token]` 拿到「全局扁平 slot」，再除/模 block_size 拆成「物理块号 + 块内偏移」落盘（`slot<0` 的 padding token 跳过）。这对应贯穿示例阶段③把 `k0,v0…` 写进 slot14/15/6 那一步——本文从 kernel 内部看它。
...
> **这一步在干嘛**：启动时决定 KV cache 能开多大。办法是**真跑一次最大 batch 的 dummy forward** 量出显存峰值（含 NCCL/IPC 这些不走 torch allocator 的分配），用「总显存 × 利用率 − 峰值」算出留给 KV 的字节数，再切成等大 block、一次性 `torch.zeros` 分配出整个 KV 池；顺便捕获 CUDA graph、准备好 pinned buffer 做异步 H2D。这些都是「一步 forward 真正开跑之前」的准备工作。

...
> **这一步在干嘛**：HTTP 请求刚到门口，把它交给引擎客户端，换回一个"异步生成器"——之后每生成一个 token，就从这个生成器里流式吐一条出来。
...
> **这一步在干嘛**：在前端进程里把用户的文本 **tokenize**（切成 token id）、校验采样参数、处理多模态，打包成一个跨进程的载荷 `EngineCoreRequest`；同时给这条请求建一个专属的输出队列，等会儿生成的 token 都往这里塞。
...
> **这一步在干嘛**：把打包好的 `EngineCoreRequest` 用 msgpack 编码、经 ZMQ socket 从前端进程**送到后端 EngineCore 进程**。后端有个专门的 IO 线程收下、解码、塞进进程内队列——收发时释放 GIL，与 GPU forward 重叠，不占关键路径。
...
> **这一步在干嘛**：后端进程的"心脏"——一个纯同步的 `while True` 循环，反复"取输入 → 干一步活（`step()`）→ 把输出塞队列"。新请求先被转成内部状态机对象 `Request`、追加进调度器的 `waiting` 队列，等着被调度。
...
> **这一步在干嘛**：`step()` 三段式的第一段——只动脑不动手。决定"这一步给哪些请求、各算多少 token、用哪些 KV block"，产出一个纯数据对象 `SchedulerOutput`（详细算法见 [模块 01](../01-scheduler-batching/design.md)）。
>
> 🔗 **回扣[贯穿示例](../PRIMER.md#第三部分一个贯穿全文的玩具示例强烈建议先看懂这个)**：例子里请求 `[11,7,4]` 的 prefill 步在这里被决定"算 3 个 token"，decode 步被决定"算 1 个 token（id=9 那个）"。
...
> **这一步在干嘛**：`step()` 三段式的第二段——真正动手。把调度产物 `SchedulerOutput` 下发到 GPU，跑一遍模型 forward 算出 logits、再采样出下一个 token。单卡直接调用，多卡则经 Executor 广播给各 Worker（见 [模块 03](../03-distributed-parallel/design.md)）。
>
> 🔗 **回扣[贯穿示例](../PRIMER.md#第三部分一个贯穿全文的玩具示例强烈建议先看懂这个)**：这一步就是例子阶段②~④——算 Q/K/V、attention、写 KV 进分页缓存、最后采样出 token id=9。
...
> **这一步在干嘛**：`step()` 三段式的第三段——GPU 算完回来后收尾。把新生成的 token 接到各请求后面、检查谁该停（命中 EOS / 长度上限），打包成 `EngineCoreOutputs` 准备送回前端。
...
> **这一步在干嘛**：把后端这一步的输出（新 token + 是否结束）用 msgpack 编码、经 ZMQ 从后端进程**送回前端进程**。同样由一个独立 IO 线程负责发送，和 GPU forward 重叠。
...
> **这一步在干嘛**：前端唯一的后台 loop 拉到一批输出，把 token id **detokenize** 成文本，按 request_id 分发到各自的输出队列。然后阶段 A 那个生成器就能拿到文本、转成 SSE chunk 流式发回客户端——一条请求的旅程到此闭环。
>
> 🔗 **回扣[贯穿示例](../PRIMER.md#第三部分一个贯穿全文的玩具示例强烈建议先看懂这个)**：采样出的 token id=9 在这里被 detokenize 成对应的文本片段，push 进该请求的队列，最终流式吐回客户端。
